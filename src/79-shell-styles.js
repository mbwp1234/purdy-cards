...
const PS_STYLES = `
      ...
      * { box-sizing: border-box; }
      ...
      .ps-col {
        ...
        /* Two layers. The second is the glass itself; the first is a scrim at
           the FOOT of the column, because the horizon now sits behind exactly
           that strip and --ps-dim measured 3.92:1 over the ember there against
           5.40:1 mid-column — the smallest text going under the 4.5:1 floor,
           the same rule that set --ps-dim in the first place. It is sized in
           PIXELS, not a percentage: the column's height changes with every
           expansion, and a percentage would spread the scrim over a third of
           an open column. The band stays untouched in the margins and behind
           the dock, which is where it is meant to be read. */
        background:
          linear-gradient(0deg, rgba(9,11,20,.52), transparent 140px),
          linear-gradient(180deg, rgba(14,18,32,.34), rgba(9,11,20,.28));
      ...
      /* The horizon glow the dock rests on rides the same pseudo as the fade —
         ...
         It must not read as a BOX. It used to: -12px against a 6px host
         padding made it wider than the card, so overflow-x: clip cut it into
         hard vertical edges; the bottom edge landed exactly on the card's bottom
         padding edge and the gradient was still at .68 when it got there, so
         the foot was a hard edge too. The result was a dark slab around the
         dock — the dock's own fill on top of a second background, which is
         what "two backgrounds" in the bottom box was.

         The fade's only job is masking column content that slides UNDER the
         dock, and content only ever comes from above. So it is strongest at
         the dock and falls to nothing on all four sides: transparent at the
         bottom (there is never anything down there to hide, and the ground's
         hem already owns that strip), and masked at the edges so it dissolves
         into the sky instead of ending at one. */
      .ps-dockwrap::before {
        content: ""; position: absolute; z-index: -1; pointer-events: none;
        left: -10px; right: -10px; top: -80px;
        bottom: calc(-12px - env(safe-area-inset-bottom, 0px));
        background:
          radial-gradient(120% 86% at 50% 96%, var(--ps-sky-glow), transparent 72%),
          linear-gradient(180deg, transparent, rgba(6,7,14,.44) 54%, rgba(6,7,14,.22) 88%, transparent);
